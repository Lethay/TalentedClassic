...
function Talented:MakeTemplateMenu()
    ...
    -- 	table.sort(menuList, Sort_Template_Menu_Entry)
    -- end
    -- Old: local talentGroup = GetActiveTalentGroup()
    local talentGroup = C_SpecializationInfo and C_SpecializationInfo.GetActiveSpecGroup() or GetActiveTalentGroup()
    local entry = self:GetNamedMenu("primary")
...
function Talented:MakeActionMenu()
    ...
    local templateTalentGroup = self.template.talentGroup
    local activeTalentGroup =
        C_SpecializationInfo and C_SpecializationInfo.GetActiveSpecGroup() or GetActiveTalentGroup()
    ...
    self:GetNamedMenu("Delete").disabled =
        templateTalentGroup or not self.db.global.templates[self.template.class][self.template.name]
    ...
    if templateTalentGroup then
        ...
        target.checked =
            (self.db.char.targets[targetName] and self.db.char.targets[targetName].name == self.template.name)
    end
