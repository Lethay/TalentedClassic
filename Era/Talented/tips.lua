...
local function gettipline(tip, tipValues, rank)
    ...
    if not tipValues or #tipValues == 0 then
        return tip
    end

    if not tipValues[rank] then
        return tip
    end

    return tip:format(unpack(tipValues[rank]))
end
...
function Talented:SetTooltipInfo(frame, class, tab, index)
    ...
    if IsAltKeyDown() then
        for i = 1, info.ranks do
            ...
            addtipline(
                gettipline(info.tips, info.tipValues, i),
                i == rank and HIGHLIGHT_FONT_COLOR or NORMAL_FONT_COLOR
            )
        end
    else
        if rank > 0 then
